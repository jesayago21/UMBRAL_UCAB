...
import { usePreguntasTriviaSesionEquipo } from '@/hooks/useSesiones'
...
interface EquipoTriviaGameplayShellProps {
...
export function EquipoTriviaGameplayShell({
  inscripcion,
  onSalir,
}: EquipoTriviaGameplayShellProps) {
  const {
    data: preguntas,
    isLoading,
    isError,
    error,
    refetch,
  } = usePreguntasTriviaSesionEquipo(inscripcion.sesionId)
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-center text-xs text-slate-500">
        <Link to={rutaListadoEquipo('Trivia')} className="text-indigo-600 hover:underline">
